...
You can check the names of columns and the first columns by using the next commands. We will use the `groundbeef` data set. This data set contains one variable `serving` which represents the serving sizes collected in a French survey, for ground beef patties consumed by children under 5 years old. This data set is provided within the *fitdistrplus* package.
...
v_norm_2 <- rnorm(100, 5, 1.5)
v_norm_3 <- rnorm(100, 9, 0.75)
...
Often, you will only dispose of the summary statistics (mean and standard deviation) of a distribution instead of the individual patient level data to fit these statistical distributions. In those case, the method of moments can be used to calculate the parameters of the distribution based on these summary statistics.
...
dist <- get.beta.par(p = c(0.25, 0.5, 0.75), q = v_quantiles) # fit beta distribution based on quantiles
dist
```
...
surv_expo <- flexsurvreg(km~group, data = bc, dist = "expo")
surv_weib <- flexsurvreg(km~group, data = bc, dist = "weibull")
surv_gamma <- flexsurvreg(km~group, data = bc, dist = "gamma")
