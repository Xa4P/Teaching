...
### Discount costs and effects
Apply discounting on the incremental effects and costs to convert these to their present value. Assign the discounted effects and costs to new columns of `df_thx` called `Inc_cost_A_d`, `Inc_QALY_A_d`, `Inc_cost_B_d`, `Inc_QALY_B_d`, `Inc_cost_C_d`, `Inc_QALY_C_d`.  

```{r cacl_disc_inc, exercise = TRUE}
df_thx$Inc_cost_A_d
df_thx$Inc_QALY_A_d
df_thx$Inc_cost_B_d
df_thx$Inc_QALY_B_d
...
Calculate the total **discounted** incremental costs and effects, and ICERs for each intervention, store the incrementals in the vector `v_res_disc`, and the ICERs in the vector `v_icer_disc`.
```{r, include = FALSE}
df_thx$Inc_cost_A_d <- df_thx$Inc_cost_A * v_disc
df_thx$Inc_QALY_A_d <- df_thx$Inc_QALY_A * v_disc
df_thx$Inc_cost_B_d <- df_thx$Inc_cost_B * v_disc
