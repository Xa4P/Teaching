...
2. Open the `Assignment_start.R` file and follow the instructions of this document or in the R file (these are the same). Use the model structure on the enxt page to perform the assignment.       
...
1. Use the model parameters (the `p_[NAME] parameters`) defined in the `Assignment_start.R` and the model structure to calculate the following probabilities/quantities (subquestions a to j).  
...
4. Re-open your `Assignment_start.R` file to perform the cohort simulation.    
...
*HINT:* to do these calculation automatically, use a a for loop over rows 1 to `n_cycles` of the `m_hs` matrix. 4.c. Use your cohort simulation to calculate the number of individuals being alive after exactly 3 years. Remember that individuals only remain alive if they are in one of the three health states “Healthy” , “ NewAneurysm”  and “ Detected Aneurysm”.  
